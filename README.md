...
`/resume` opens the real ChatGPT conversation catalog, supports fuzzy filtering, renders the selected conversation's user-visible history, and continues that same conversation. `/resume <URL-or-ID>` skips the picker. An attached chat header shows its full `https://chatgpt.com/c/...` URL, and a new chat prints that URL as soon as the first completed response supplies its id. During an enhanced interactive turn, a live `elapsed MM:SS`/`HH:MM:SS` timer stays at the bottom while thinking/tool output prints above it; the elapsed value freezes when the final answer arrives. A completed interactive response also triggers a best-effort native macOS notification via `osascript`; the title includes the tail of the conversation ID and the body contains the latest user prompt, normalized and length-bounded, so concurrent chat notifications are distinguishable. Successful `/new`, `/resume`, and `/detach` clear only the current viewport before rendering the new context; terminal scrollback remains available. The default model policy is `latest frontier · High`: text-only turns use ChatGPT's native `DEEP/HIGH` product profile. Rich-input turns cannot currently combine CWA's profile selector with attachments, so gptty resolves the live model catalog once per session and uses the strongest normal non-Work thinking frontier slug for default image turns; currently this is GPT-5.6 Sol High. `/model` opens the live normal-chat model catalog for explicit overrides, and `/model default` returns to this latest-frontier High policy. `/detach` only clears the local attachment and does not modify the ChatGPT chat.
