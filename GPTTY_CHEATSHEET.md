...
/goal list             показать все незавершённые Goal этого profile
/goal list all         показать также завершённые/исторические Goal
/goal open <id>        переключиться на Goal по уникальному префиксу id
/goal pause            приостановить автопродолжение текущего Goal
/goal resume           продолжить текущий paused/blocked Goal
/goal status           показать состояние/счётчики текущего Goal
/goal clear            отвязать текущий Goal от его чатов, сохранив историю
```

В одном profile теперь может существовать много Goal и несколько разных Goal могут быть `active` одновременно в разных gptty-процессах. Ограничение ownership действует на конкретный Goal: у Goal максимум один live owner, но Goal A и Goal B друг другу не мешают. Один normal conversation относится максимум к одному незавершённому Goal; один Goal может владеть несколькими conversation generations после rollover. `/goal list` — основной обзор, `/goal open <id>` безопасно переключает текущий TUI на нужный Goal, а `/resume` picker помечает чаты их Goal status/id. GoalStore общий для profile (`goals/goal-state.sqlite3`), а локальный `current_conversation` у каждого интерактивного terminal surface свой: рядом с base `gptty_state.json` создаётся `gptty_state.session-<source>-<hash>.json`. В cmux `source` строится из `CMUX_SURFACE_ID`, поэтому два surface не перетирают selection/state друг друга; `GPTTY_SESSION_ID` можно задать явно. Base-файл остаётся legacy seed/non-interactive state. Потеря terminal-local state не выбирает случайный Goal: все durable Goal остаются в SQLite и доступны через list/open. В Goal mode финальный ответ каждого turn начинается с `GPTTY_GOAL: CONTINUE`, `GPTTY_GOAL: COMPLETE` или `GPTTY_GOAL: BLOCKED` и структурированного checkpoint. Авторитетное состояние и routing хранятся транзакционно в `goals/goal-state.sqlite3`; `goals/index.json`, `goals/<goal-id>/goal.json`, `checkpoint.md` и `events.jsonl` — переносимые читаемые проекции/backup, а не источник concurrency truth. Перед каждым turn gptty фиксирует durable operation id, user steering, committed conversation identity и наблюдаемые tool call/result в append-only journal. `COMPLETE` не принимается без непустого verified `completed`, при непустом `pending` или пока остаётся tool call без наблюдаемого result. Если старый write стал ambiguous, recovery не повторяет его вслепую: сначала нужна отдельная наблюдаемая verification call/result, после чего reconciliation вместе со structured checkpoint фиксируется в journal. При мёртвом/исчерпанном ChatGPT-чате Goal делает Continue-As-New: тот же Goal ID, новая generation и новый conversation; старые conversation bindings остаются частью того же Goal. `Ctrl-C`/`/stop`, локальный выход и restart pause/reconcile только соответствующий Goal и не останавливают другие Goal profile. Goal mode намеренно не работает внутри Temporary Chat.
...
- после выбора `/resume` делает ровно один canonical snapshot как неблокирующую resume-operation: composer остаётся доступным, обычный текст ставится в FIFO до окончания snapshot, а `/exit`/`Ctrl-\\` могут выйти сразу; `tool_running`/`tool_calling` у старого незавершённого turn показываются как `unfinished` и не запускают polling/follow;
