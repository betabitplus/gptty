...
`/goal` turns the current normal ChatGPT conversation into a persistent run-until-done task. On an attached chat, bare `/goal` immediately continues the task/plan already established in that conversation; `/goal <objective>` can also start a new normal chat with an explicit objective. Goal turns must begin their final assistant response with exactly `GPTTY_GOAL: CONTINUE`, `GPTTY_GOAL: COMPLETE`, or `GPTTY_GOAL: BLOCKED`. `CONTINUE` automatically sends the next continuation turn in the same conversation without a completion notification. `COMPLETE` stops the loop and sends the normal final notification. `BLOCKED` stops the loop and notifies that user action is required. A missing/invalid status is treated as unfinished work and retried with protocol recovery; three consecutive protocol misses interrupt the goal rather than loop forever. `Ctrl-C` during a goal turn performs the normal remote Stop and also pauses the goal, while `Ctrl-\\` exits locally, lets the remote ChatGPT turn continue, and leaves the local goal paused for explicit `/goal resume`. Active goals are also paused across process restart and when switching/detaching conversations; they never resume silently in the background. Use `/goal pause`, `/goal resume`, `/goal status`, and `/goal clear` for explicit control. Goal mode is intentionally unavailable in Temporary Chat because its continuation authority is not persistent.
