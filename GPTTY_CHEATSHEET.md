...
В Goal mode финальный ответ каждого turn начинается с `GPTTY_GOAL: CONTINUE`, `GPTTY_GOAL: COMPLETE` или `GPTTY_GOAL: BLOCKED` и структурированного checkpoint. Авторитетное состояние хранится транзакционно в `goals/goal-state.sqlite3` рядом с `gptty_state.json`; `goals/<goal-id>/goal.json`, `checkpoint.md` и `events.jsonl` — переносимые читаемые проекции/backup, а не источник concurrency truth. Перед каждым turn gptty сначала фиксирует durable operation id, user steering и наблюдаемые tool call/result в append-only journal. `COMPLETE` не принимается без непустого verified `completed`, при непустом `pending` или пока остаётся tool call без наблюдаемого result. Если старый write стал ambiguous, recovery не повторяет его вслепую: сначала нужна отдельная наблюдаемая verification call/result, после чего reconciliation вместе со structured checkpoint фиксируется в journal. При мёртвом/исчерпанном ChatGPT-чате Goal делает Continue-As-New: тот же Goal ID, новая generation и новый conversation, с checkpoint + machine journal в handoff. `CONTINUE` запускает следующий turn без финального notification; `BLOCKED` останавливает цикл и сообщает, что нужен пользователь. Отсутствующий/битый status считается незавершённой работой и проходит recovery. Во время Goal turn обычный введённый текст становится queued steering и сохраняется до отправки. `Ctrl-C`/`/stop`, локальный выход и restart не дают Goal самопроизвольно ехать дальше: он pause/reconcile и требует безопасного resume. Для одного state/profile одновременно допускается один live owner Goal; второй gptty не может отправлять Goal-сообщения или менять conversation/model, пока владелец жив. Goal mode намеренно не работает внутри Temporary Chat.
