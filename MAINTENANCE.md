...
The downstream interactive UI is intentionally isolated under `src/gptty/ui/`: `prompt_toolkit` owns input/history/menus and Rich owns line-oriented presentation. Keep normal terminal scrollback; successful `/new`, `/temporary`, `/resume`, and `/detach` may clear the current viewport (`CSI 2 J` + home) to establish a fresh visual context, but must not erase scrollback or switch to an alternate screen. Attached normal-chat headers must expose the full canonical ChatGPT URL. Temporary Chat lifecycle authority remains entirely in CWA: gptty may select `conversation_mode="temporary"` and retain only the current user-visible transcript for local rendering/export, but must never persist the temporary conversation id as resumable state or recreate continuation authority from it. Context switches and normal `/exit` explicitly end a live temporary lifecycle; the `Ctrl-\\` local-only escape must not end it because the browser response is intentionally allowed to continue. `/export` is a local presentation operation: canonical normal-chat history comes from CWA, while the current live Temporary Chat uses the in-memory visible transcript; generated Markdown files go to the user's local export directory and must never overwrite an existing export. Pending image attachments are one-turn local UI state: file paths stay user-owned, clipboard images are materialized with native macOS `osascript` pasteboard PNG coercion into a session temp directory, and only gptty-owned clipboard files may be deleted. Context switches clear pending attachments to prevent cross-chat sends. Do not add clipboard polling or require `pngpaste`/Homebrew while the native path works. The generation timer is presentation-only: one Rich live line may refresh locally once per second while a turn/follow is active, with no added ChatGPT network polling. During an enhanced turn `Ctrl-C` means remote Stop generating, not local SDK-stack interruption: the blocking CWA send stays in a worker thread, the main TUI thread sends CWA's out-of-band stop request, then waits for the original send/readback path to reconcile before rendering the partial answer. `/stop` uses the same CWA primitive. `Ctrl-\\` is the explicit local-only escape hatch: it exits gptty without sending Stop, allowing the browser/native turn to continue; if requested before `browser_native_write_completed`, process exit must be deferred until that safe handoff event arrives. Do not duplicate DOM stop selectors or browser cancellation logic in gptty. Completion notification is best-effort local macOS `osascript`; notification failure must never fail a chat turn, and user-stopped turns must not generate a completion notification. Goal mode is also a gptty-only orchestration concern: it persists one goal bound to one normal conversation, decides whether a completed model turn means continue/complete/blocked from the explicit `GPTTY_GOAL` marker, and may enqueue the next ordinary CWA send only after the previous turn has fully returned. It must not add transport polling, browser hooks, background goal workers, or cross-chat migration to CWA. Intermediate Goal turns suppress completion notifications; only complete, blocked, or interrupted terminal goal states notify. User Stop, local-only exit, restart, and conversation switches must fail safe by pausing the goal rather than silently continuing it. Do not introduce an alternate-screen/full-screen TUI unless there is a separately proven need. The non-TTY and `--plain` paths must remain compatible with upstream CLI/scripting behavior. If upstream gains equivalent input, palette, rendering, or goal-orchestration features, prefer upstream and remove the replaced downstream code instead of maintaining two implementations.
